...
def update_server(name, hostname, port, username, password, domain):
    try:
        ...
        stdin, stdout, stderr = client.exec_command('wget -O /home/web/nginx.conf https://raw.githubusercontent.com/jaraim/My-project/main/docker/nginx/nginx.conf && sed -i "s/yuming.com/' + domain + '/g" /home/web/nginx.conf')
        while not stdout.channel.exit_status_ready():
            if stdout.channel.recv_ready():
                print(stdout.channel.recv(1024).decode(), end="")
